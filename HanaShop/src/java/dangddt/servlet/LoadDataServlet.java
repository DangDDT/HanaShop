...
public class LoadDataServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Vector<ProductDTO> result = ProductDAO.loadProduct(pageIndexInt);
            max_product = ProductDAO.countPro();
            page = (int) Math.ceil((double) max_product / MAX_PRODUCT_IN_PAGE);
            ...
            if (session != null) {
                Vector<ProductDTO> favourite_products = null;
                if (session.getAttribute("ACCOUNT") instanceof GooglePojo) {
                    GooglePojo dto = (GooglePojo) session.getAttribute("ACCOUNT");
                    if (dto != null) {
                        String userID = dto.getUserID().trim();
                        favourite_products = ProductDAO.loadFavoriteProducts(userID);
                    }
                } else {
                ...
                session.setAttribute("FAVOURITE_PRODUCTS", favourite_products);
            }
